...
export const sections: Section[] = [
  ...
  { id: 'ai-coding', title: 'AI Coding Patterns', path: '/ai-coding', description: 'Patterns for building with coding agents.', icon: AutoAwesomeOutlined, pages: [] },
  {
    ...
    title: 'Python',
    path: '/python',
    description: 'Language internals, async, packaging and the stdlib.',
...
export const searchIndex: SearchItem[] = [
  ...
  ...manifest.python.map((n) => ({
    key: `py:${n.slug}`,
    label: n.title,
    secondary: `Python · ${n.sectionTitle} · ${n.tier}`,
    path: `/python/${n.slug}`,
    group: 'Python',
  })),
  ...manifest.networking.map((n) => ({
  ...
  ...manifest.gameDay.map((t) => ({
    key: `gameday:${t.slug}`,
    label: t.title,
    secondary: `Game Day · ${t.count} questions`,
    path: `/game-day/recall/${t.slug}`,
    group: 'Game Day',
  })),
  ...playbookIndex.map((s) => ({
